        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == 1) { // Botón Devolver
                ...
                try {
                    ...
                    result r = txn.exec_params("SELECT estado FROM prestamos WHERE isbn = $1 ORDER BY id DESC LIMIT 1", std::string(isbn));
                    ...
                    if (r.empty()) {
                        ...
                        if (estado == "prestado") {
                            ...
                            if (!libro.empty()) {
                                ...
                                if (respuesta == IDYES) {
                                    txn.exec_params("UPDATE prestamos SET estado = 'disponible' WHERE isbn = $1 AND estado = 'prestado'", std::string(isbn));
                                    txn.commit();
